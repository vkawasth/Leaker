...
# https://towardsdatascience.com/transformer-aided-supply-chain-network-design-fe45bb846f0f/
# Adaptive attention span https://arxiv.org/pdf/1905.07799
# Multiplication - https://arxiv.org/pdf/2510.00184v1
# COT - https://arxiv.org/pdf/2402.12875
# LITE TRANSFORMER with LONG SHORT TERM ATTENTION - https://arxiv.org/pdf/2004.11886
# Timeseries Transformer (AUTOFORMER) - https://arxiv.org/pdf/2106.13008
# 
...
tgt_data_sim = torch.randint(1000, tgt_vocab_size, (6400, max_seq_length))
src_data_low = torch.randint(10, 500, (6400, max_seq_length))
tgt_data_low = torch.randint(10, 500, (6400, max_seq_length))
max_seq_length_h = max_seq_length // 2
src_data_sim_h = torch.randint(1000, src_vocab_size, (6400, max_seq_length_h ))
tgt_data_sim_h = torch.randint(1000, tgt_vocab_size, (6400, max_seq_length_h ))
src_data_low_h = torch.randint(10, 500, (6400, max_seq_length_h ))
tgt_data_low_h = torch.randint(10, 500, (6400, max_seq_length_h ))
src_data_hl = torch.cat((src_data_sim_h, src_data_low_h), 1)
tgt_data_hl = torch.cat((tgt_data_sim_h, tgt_data_low_h), 1)
...
src_dict = torch.cat((src_dict[:,:0], keys_sim, src_dict[:,0:]), dim=1)
# Input Tree Type
...
tgt_dict = torch.cat((tgt_dict[:,:0], keys_sim, tgt_dict[:,0:]), dim=1)
tgt_dict = torch.cat((tgt_dict[:,:5], ones, tgt_dict[:,5:]), dim=1) # KEY IS IN
tgt_dict_sim = torch.cat((tgt_dict[:,:6], END, tgt_dict[:,6:]), dim=1) # KEY IS IN

num_keys=6400
keys_low = torch.randint(low=200, high=300, size=(num_keys,1))
src_dict = torch.cat((src_data_low[:,:0], START, src_data_low[:,0:]), dim=1)
src_dict = torch.cat((src_dict[:,:0], keys_low, src_dict[:,0:]), dim=1)
# Input Tree Type
#src_dict = torch.cat((src_dict[:,:5], zeros, src_dict[:,5:]), dim=1)
src_dict_low = torch.cat((src_dict[:,:6], END, src_dict[:,6:]), dim=1)

tgt_dict = torch.cat((tgt_data_low[:,:0], START, tgt_data_low[:,0:]), dim=1)
tgt_dict = torch.cat((tgt_dict[:,:0], keys_low, tgt_dict[:,0:]), dim=1)
tgt_dict = torch.cat((tgt_dict[:,:5], threes, tgt_dict[:,5:]), dim=1) # KEY IS IN
tgt_dict_low = torch.cat((tgt_dict[:,:6], END, tgt_dict[:,6:]), dim=1)
...
tgt_dict = torch.cat((tgt_dict[:,:0], keys_low, tgt_dict[:,0:]), dim=1)
tgt_dict = torch.cat((tgt_dict[:,:5], fours, tgt_dict[:,5:]), dim=1) # KEY IS IN
tgt_dict_hl = torch.cat((tgt_dict[:,:6], END, tgt_dict[:,6:]), dim=1) 
...
torch.save(src_dict_shuf, 'src1.pt')
torch.save(tgt_dict_shuf, 'tgt1.pt')
src1_dict_shufl = torch.load('src1.pt')
tgt1_dict_shufl = torch.load('tgt1.pt')
#print(src1_dict_shufl)
#print(tgt1_dict_shufl)

batch_size=64
shuffle = True
num_workers = 4

data_loader_src = DataLoader(src1_dict_shufl, batch_size = batch_size, shuffle=shuffle, num_workers = num_workers)
data_loader_tgt = DataLoader(tgt1_dict_shufl, batch_size = batch_size, shuffle=shuffle, num_workers = num_workers)
# Create Iterators
...
for epoch in range(50):
    ...
    for _ in range(100): # 6400 data/64 batch = 100 batches
        ...
        optimizer.zero_grad()
        out = transformer(src_data_batch, dst_data_batch[:, :-1])
...
val_src_data_s = torch.randint(1000, src_vocab_size, (1600, max_seq_length))
val_tgt_data_s = torch.randint(1000, tgt_vocab_size, (1600, max_seq_length))
val_src_data_l = torch.randint(10, 500, (1600, max_seq_length))
val_tgt_data_l = torch.randint(10, 500, (1600, max_seq_length))

#high low sequence

val_src_data_hi = torch.randint(1000, src_vocab_size, (1600, max_seq_length_h))
val_tgt_data_hi = torch.randint(1000, tgt_vocab_size, (1600, max_seq_length_h))
val_src_data_lo = torch.randint(10, 500, (1600, max_seq_length_h))
val_tgt_data_lo = torch.randint(10, 500, (1600, max_seq_length_h))
...
val_tgt_dict = torch.cat((val_tgt_dict[:,:5], ones, val_tgt_dict[:,5:]), dim=1)
val_tgt_dict_sim = torch.cat((val_tgt_dict[:,:6], END, val_tgt_dict[:,6:]), dim=1)
...
val_tgt_dict = torch.cat((val_tgt_dict[:,:0], keys, val_tgt_dict[:,0:]), dim=1)
val_tgt_dict = torch.cat((val_tgt_dict[:,:5], threes, val_tgt_dict[:,5:]), dim=1)
val_tgt_dict_low = torch.cat((val_tgt_dict[:,:6], END, val_tgt_dict[:,6:]), dim=1)
...
val_tgt_dict = torch.cat((val_tgt_dict[:,:0], keys, val_tgt_dict[:,0:]), dim=1)
val_tgt_dict = torch.cat((val_tgt_dict[:,:5], fours, val_tgt_dict[:,5:]), dim=1)
val_tgt_dict_hl = torch.cat((val_tgt_dict[:,:6], END, val_tgt_dict[:,6:]), dim=1)
...
with torch.no_grad():
    val_output = transformer(val_src_dict_shuf, val_tgt_dict_shuf[:, :-1])
